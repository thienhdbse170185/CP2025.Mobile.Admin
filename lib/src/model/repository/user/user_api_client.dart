...
import 'package:smart_farm_admin/src/model/response/server_time/get_server_time_response.dart';
...
class UserApiClient {
  ...
  Future<bool> updateDeviceToken(String userId, String deviceToken) async {
    try {
      final response = await dio.put(
        '${ApiEndpoints.getUsers}/$userId/device',
  ...
  Future<bool> sendOTPSms(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        log(
          '[Server Send_OTP]: Server đã gửi OTP đến số điện thoại $phoneNumber',
        );
        return true;
      } else {
  ...
  Future<bool> verifyOTP({required String email, required String otp}) async {
    try {
      ...
      final response = await dio.post(
        '${ApiEndpoints.getUsers}/otp/verify',
        data: request,
      );
      if (response.statusCode == 200) {
        return true;
