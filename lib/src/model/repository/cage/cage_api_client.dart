...
import 'package:smart_farm_admin/api_endpoints.dart';
import 'package:smart_farm_admin/src/model/dto/cage_admin/cage_admin_dto.dart';
import 'package:smart_farm_admin/src/model/entity/cage/cage.dart';
import 'package:smart_farm_admin/src/model/response/cage/get_all/get_all_cage_response.dart';
...
class CageApiClient {
  ...
  Future<CageAdminDto> getCageAdminById({required String cageId}) async {
    try {
      ...
      final response = await dio.get(
        '${ApiEndpoints.farmingBatch}/$cageId/current-farming-stage',
      );
  ...
  Future<List<Cage>> fetchCagesByUserId(String userId) async {
    try {
      log(
        '[CAGE_API_CLIENT] Đang call API lấy thông tin chuồng theo userId $userId',
      );
      final response = await dio.get('${ApiEndpoints.getUsers}/$userId/cages');
      if (response.statusCode == 200) {
        log(
          '[CAGE_API_CLIENT] Lấy thông tin chuồng theo userId $userId thành công',
        );
        ...
        log(
          '[CAGE_API_CLIENT] Lỗi không xác định khi lấy thông tin chuồng theo userId $userId',
        );
        throw Exception('get-cage-by-user-id-failed');
      }
    } on DioException catch (e) {
      log(
        '[CAGE_API_CLIENT] Lấy thông tin chuồng theo userId $userId thất bại',
      );
      log('[CAGE_API_CLIENT] Error: $e');
